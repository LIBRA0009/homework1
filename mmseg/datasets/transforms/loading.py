...
@TRANSFORMS.register_module()
class LoadSUIMDepthAsChannel(BaseTransform):
    """Load SUIM offline depth and concatenate it as the fourth image channel.

    The depth files are expected to be generated by
    ``tools/generate_suim_depth.py`` and stored as ``float32 .npy`` files under
    ``data/SUIM/depth/{split}/{stem}.npy``.
    ...
    - img: RGBD image with shape (H, W, 4), float32
    ...
    def _get_depth_path(self, img_path: str) -> str:
        ...
        if path.parent.parent.name != 'images':
            raise ValueError(
                'Expected SUIM image path like '
                f'.../images/{{split}}/{{name}}, but got {img_path}')
        split = path.parent.name
        root = path.parent.parent.parent
        return str(root / self.depth_root / split / f'{path.stem}.npy')
    ...
    def transform(self, results: dict) -> dict:
        ...
        depth_path = self._get_depth_path(results['img_path'])
        depth = np.load(depth_path).astype(np.float32)
        if depth.shape[:2] != img.shape[:2]:
            depth = mmcv.imresize(
                depth, (img.shape[1], img.shape[0]), interpolation='bilinear')
        depth = np.clip(depth, 0.0, 1.0) * self.depth_scale
        depth = depth[..., None].astype(np.float32)

        results['img'] = np.concatenate([img, depth], axis=2)
        results['depth_map_path'] = depth_path
        return results

    def __repr__(self) -> str:
        return (f'{self.__class__.__name__}(depth_root={self.depth_root}, '
                f'depth_scale={self.depth_scale}, '
                f'convert_bgr_to_rgb={self.convert_bgr_to_rgb})')
